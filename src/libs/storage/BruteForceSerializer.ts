export const objectDump = (thing: any, depth: number = 0) => {
  ...
  if (thing.toLowerCase) {
    return `'${thing}'`;
  }
  if (Number.isFinite(thing)) {
    return `${thing}`;
  }
...
export class BruteForceSerializer {
  ...
  private normalize(
    normalizeMe: any,
    path: string,
    lookup: Map<object, number> | undefined = undefined,
  ) {
    ...
    switch (myType) {
      ...
      case "undefined":
        return null;
      default:
        return `Unhandled object type: ${myType}`;
    }
    ...
    if (normalizeMe instanceof Map) {
      ...
      Array.from(normalizeMe.keys()).forEach((k) =>
        output.__kv.push([
          this.normalize(k, `${path}.key:`, lookup),
          this.normalize(normalizeMe.get(k), `${path}.value`, lookup),
        ]),
      );
      return output;
    }
    ...
    if (normalizeMe instanceof Set) {
      output.__kv = Array.from(normalizeMe.values()).map((v) =>
        this.normalize(v, `${path}.value:`, lookup),
      );
      return output;
    }
