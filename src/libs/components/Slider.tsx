  ...
  render() {
    ...
    const onDragEnd = (ev: TouchableDragEndEvent) => {
      ...
      const drift = () => {
        ...
        const frameMagnitude = Math.sqrt(
          frameMomentum.x * frameMomentum.x + frameMomentum.y * frameMomentum.y,
        );
        ...
        if (frameMagnitude > (this.props.dragStartDistance ?? 1)) {
          action(() => {
            ...
            if (this.props.onDrift) {
              this.props.onDrift({
                momentum: frameMomentum,
                delta: { x: dx, y: dy },
                offset: { x: this.st.offsetX, y: this.st.offsetY },
              });
            }
