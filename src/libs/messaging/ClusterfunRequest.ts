...
export default class ClusterfunRequest<REQUEST, RESPONSE> implements PromiseLike<RESPONSE> {
  ...
  private timerHandler = () => {
    ...
    const totalLifetime =
      "suggestedTotalLifetimeMs" in this.endpoint ? this.endpoint.suggestedTotalLifetimeMs! : 30000;
    if (this._startTime + totalLifetime < now) {
      this.reject(
        new Error(`Could not receive a response for ${this.endpoint.route} from ${this.receiver}`),
      );
      return;
    }
    const retryInterval =
      "suggestedRetryIntervalMs" in this.endpoint
        ? this.endpoint.suggestedRetryIntervalMs!
