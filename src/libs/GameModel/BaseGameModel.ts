...
export abstract class BaseGameModel {
  ...
  get gameTime_ms() {
    return this._gameTime_ms;
  }
  set gameTime_ms(value) {
    action(() => {
      this._gameTime_ms = value;
    })();
  }
  ...
  public get devPause() {
    return this._devPause;
  }
  public set devPause(value: boolean) {
    action(() => {
      this._devPause = value;
    })();
  }
  ...
  public get devFast() {
    return this._devFast;
  }
  public set devFast(value: boolean) {
    action(() => {
      this._devFast = value;
    })();
  }
  ...
  protected onSessionClosed(code: number) {
    ...
    if (code !== CLOSECODE_PAGEREFRESH && code !== CLOSECODE_PLEASERETRY) {
  ...
  saveCheckpoint() {
    ...
    if (this._isCheckpointing) {
      ...
      if (timeSinceLastSuccess > 3000) {
        Logger.warn(
          `WEIRD: Last checkpoint did not appear to finish.  Took ${(timeSinceLastSuccess / 1000).toFixed(1)} seconds`,
        );
  ...
  public registerAnimation = (controller: BaseAnimationController) => {
    ...
    this.onTick.subscribe(tag, (t) => {
      ...
      if (controller.sequenceFinished && controller.animationsFinished)
        this.onTick.unsubscribe(tag);
    });
  };
