...
@inject("appModel")
@observer
class MonitoringPage extends React.Component<{ appModel?: StressatoPresenterModel }> {
  // -------------------------------------------------------------------
  // ctor
  // -------------------------------------------------------------------
  ...
  render() {
    ...
    const serverSent = appModel.serverHealth?.summary?.find((i) => i.label === "MessageSend")
      ?.data ?? { count: 0, sum: 0 };
    const serverReceive = appModel.serverHealth?.summary?.find((i) => i.label === "MessageReceive")
      ?.data ?? { count: 0, sum: 0 };
    return (
      <div>
        <Row>
        ...
        <div>Game Stats:</div>
        <div>
          Messages Sent: {appModel.session.stats.sentCount} ({appModel.session.stats.bytesSent} B)
        </div>
        <div>
          Messages Received: {appModel.session.stats.recievedCount} (
          {appModel.session.stats.bytesRecieved} B)
        </div>
        <div>
          <b>Server Stats:</b>
        </div>
        <div style={{ marginLeft: "20px" }}>
          <div>
            Rooms: {appModel.serverHealth?.rooms?.roomCount} Active:{" "}
            {appModel.serverHealth?.rooms?.activeRooms} Users:
            {appModel.serverHealth?.rooms?.activeUsers}
          </div>
          <div>
            Messages Sent: {serverSent.count} ({(serverSent.sum / 1000).toFixed(2)} KB)
          </div>
          <div>
            Messages Received: {serverReceive.count} ({(serverReceive.sum / 1000).toFixed(2)} KB)
          </div>
          <div>CPU Utilization: {(appModel.serverHealth?.cpuUsage?.user * 100).toFixed(2)}%</div>
          <div>
            Memory Utilization: {(appModel.serverHealth?.memoryUsage?.rss / 1000000).toFixed(2)} MB
          </div>
        </div>
      </div>
    );
...
export default class Presenter extends React.Component<{
  appModel?: StressatoPresenterModel;
  ...
  constructor(props: Readonly<{ appModel?: StressatoPresenterModel; uiProperties: UIProperties }>) {
    ...
    appModel?.onTick.subscribe("Timer Watcher", () => {
      ...
      if (
        appModel!.gameState === StressatoGameState.Playing &&
        timeAlertLoaded &&
