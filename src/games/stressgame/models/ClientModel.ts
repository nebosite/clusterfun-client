...
const SEED_STRING =
  "_abcdefjhijklmnopqrstuvwxyzABCDEFJHIKJLMNOPQRSTUVWXYZ012345678909876543210abcdefjhijklmnopqrstuvwxyz.".repeat(
    100,
  );
...
export class StressatoClientModel extends ClusterfunClientModel {
  ...
  get returnMessageSize() {
    return this._returnMessageSize;
  }
  set returnMessageSize(value) {
    action(() => {
      this._returnMessageSize = value;
    })();
  }
  ...
  reconstitute() {
    ...
    this.onTick.subscribe("TestClientTick", (t) => {
      if (t > this.nextSend) {
        ...
        this.nextSend = this.sendRate ? t + RATE_UNIT / this.sendRate : t + 10000000000;
      }
    });
  }
  ...
  protected async sendAction(actionData: any = null) {
    if (this.returnMessageSize) {
      await this.session.requestPresenter(StressatoPresenterRelayWithReturnEndpoint, {
        returnSize: this.returnMessageSize,
        actionData,
      });
    } else {
      this.session.sendMessageToPresenter(StressatoPresenterRelayWithoutReturnEndpoint, {
        actionData,
      });
    }
  }
