...
export class LetterGridModel {
      ...
      for (let y = 0; y < this.height; y++) {
        ...
        for (let x = 0; x < this.width; x++) {
          ...
          if (x !== block.coordinates.x || y !== block.coordinates.y) {
            throw Error(
              `Coordinates no matchy (${x},${y}) != B(${block.coordinates.x},${block.coordinates.y})`,
            );
          }
          processor(block);
        }
  ...
  populate(letterDeck: string) {
    ...
    for (let y = 0; y < this.height; y++) {
      ...
      for (let x = 0; x < this.width; x++) {
        ...
        const newBlock = new LetterBlockModel(letter === "Q" ? "Qu" : letter, new Vector2(x, y));
  ...
  getBlock(spot: Vector2) {
    ...
    if (spot.x < 0 || spot.x >= this.width || spot.y < 0 || spot.y >= this.height) {
      return null;
    }
