...
const collectionDragSpec = {
  ...
  isDragging(props: AnswercollectionProps, monitor: DragSourceMonitor) {
    return (
      monitor.getItemType() === "*" && props.context.id === monitor.getItem<{ id: number }>().id
    );
  },
};
...
// -------------------------------------------------------------------
@observer
class AnswerCollection extends React.Component<AnswercollectionProps> {
  // -------------------------------------------------------------------
  // render
  // -------------------------------------------------------------------
  render() {
      ...
      connectDragSource(
        <div className={styles.answerCollection} style={backgroundStyle}>
          ...
          <div className={styles.cardAnswerList}>
            {context.answers.map((c) => (
              <AnswerCard context={c} key={c.id} />
            ))}
          </div>
        </div>,
      ),
