...
export class RetroSpectroAnswer {
  ...
  // -------------------------------------------------------------------
  constructor(
    player: RetroSpectroPlayer | undefined = undefined,
    text: string = "***",
    answerType: string = "***",
  ) {
    this.player = player;
  ...
  return {
    ...
    getTypeName(o) {
      switch (o.constructor) {
        case RetroSpectroPlayer:
          return "RetroSpectroPlayer";
        case RetroSpectroAnswer:
          return "RetroSpectroAnswer";
        case RetroSpectroAnswerCollection:
          return "RetroSpectroAnswerCollection";
        case RetroSpectroPresenterModel:
          return "RetroSpectroPresenterModel";
      }
      return undefined;
    },
    constructType(typeName: string): any {
      switch (typeName) {
        case "RetroSpectroPlayer":
          return new RetroSpectroPlayer();
        case "RetroSpectroAnswer":
          return new RetroSpectroAnswer();
        case "RetroSpectroAnswerCollection":
          return new RetroSpectroAnswerCollection();
        case "RetroSpectroPresenterModel":
          return new RetroSpectroPresenterModel(sessionHelper, gameProps.logger, gameProps.storage);
      }
      return null;
    },
    ...
    reconstitute(typeName: string, propertyName: string, rehydratedObject: any) {
      if (typeName === "RetroSpectroPresenterModel") {
        switch (propertyName) {
          case "answerCollections":
            return observable<RetroSpectroAnswerCollection>(
              rehydratedObject as RetroSpectroAnswerCollection[],
            );
        }
      }
...
export class RetroSpectroPresenterModel extends ClusterfunPresenterModel<RetroSpectroPlayer> {
  ...
  get currentDiscussionIndex() {
    return this._currentDiscussionIndex;
  }
  set currentDiscussionIndex(value) {
    action(() => (this._currentDiscussionIndex = value))();
  }
  ...
  get hasPrev() {
    return this.currentDiscussionIndex > 0;
  }
  get hasNext() {
    return this.currentDiscussionIndex < this.answerCollections.length - 1;
  }
  get prevName() {
    return this.hasPrev
      ? (this.answerCollections[this.currentDiscussionIndex - 1].name ?? "_")
      : "";
  }
  get nextName() {
    return this.hasNext
  ...
  // -------------------------------------------------------------------
  insertCollection(
    insertBefore: RetroSpectroAnswerCollection,
    droppedItem: RetroSpectroAnswer | RetroSpectroAnswerCollection,
  ) {
  ...
  handleAnswer = (
    senderId: string,
    message: RetroSpectroAnswerMessage,
  ): RetroSpectroAnswerResponse => {
  ...
  alertPlayersOnDiscussion = () => {
    this.sendToEveryone(RetroSpectroDiscussionEndpoint, (player, isExited) => {
      const youAreInThis = this.currentDiscussion?.answers.find(
