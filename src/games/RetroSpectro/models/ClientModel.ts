    ...
    getTypeName(o: object) {
      switch (o.constructor) {
        case RetroSpectroClientModel:
          return "RetroSpectroClientModel";
      }
      return undefined;
    },
...
export class RetroSpectroClientModel extends ClusterfunClientModel {
  ...
  set currentAnswer(value: string) {
    if (value.length > SETTING_ANSWER_CHARACTER_LIMIT)
      value = value.substr(0, SETTING_ANSWER_CHARACTER_LIMIT);
  ...
  reconstitute() {
    ...
    this.listenToEndpointFromPresenter(
      RetroSpectroEndOfRoundEndpoint,
      this.handleEndOfRoundMessage,
    );
    this.listenToEndpointFromPresenter(
      RetroSpectroDiscussionEndpoint,
      this.handleDiscussionMessage,
    );
  }
