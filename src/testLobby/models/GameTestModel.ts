...
export class GameTestModel {
  ...
  get presenterSize() {
    return this._presenterSize;
  }
  set presenterSize(value: number) {
    action(() => {
      this._presenterSize = value;
      this.saveState();
    })();
  }
  ...
  get presenterModel() {
    return this._presenterModel;
  }
  set presenterModel(value) {
    action(() => {
      this._presenterModel = value;
    })();
  }
  ...
  private serverCall = <T>(url: string, payload: any): Promise<T> => {
    if (url === "/api/startgame") {
      ...
      Logger.info(
        "Terminating game with room id: " + payload.roomId + " ... " + payload.presenterSecret,
      );
